...
The output of this algorithm is a matrix representing the shortest distance from a node to other nodes. When a node is unreachable to another node, the algorithm returns infinity which signifies no path can be formed. The figure shown below  



The time complexity of this algorithm is O($n^3$) where $n$ is the number of vertices and space complexity is given by O($n^2$). Some common use cases of the APSP algorithm are
