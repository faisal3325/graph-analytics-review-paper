...
Node Clustering Coefficient is also known as Triangle Counting/Clustering Coefficient and is applied on undirected and unweighted graphs. The coefficient checks to what degree nodes in a network tend to cluster together, it does so by identifying the number of triangles in the graph which pass through each node. This measures the cohesiveness of clusters. The coefficient is classified into two types: global clustering coefficient and local clustering coefficient.

To understand global clustering coefficient, local clustering coefficient needs to be understood first. The latter of a node is the measure of the closeness of its neighbors to being a complete graph. This is measured using the triangle counting method. Global clustering coefficient is then the total of these local clustering coefficients.
...
The time complexity of node clustering coefficient is given by $O(n^3)$ where $n$ is number of nodes in the graph. This method is used to analyse social networks and web pages with a shared topic amongst them.
