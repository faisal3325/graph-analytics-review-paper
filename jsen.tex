...
\begin{table*}[]
    ...
    \begin{tabular}{l l l}
        ...
        \multirow{2}{*}{} & Parallel Breadth-First Search & \begin{tabular}[c]{@{}l@{}} \cite{8}\\
            \cite{36}   \\
            \cite{37}   \\
        \end{tabular}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Parallel Depth-First Search & \begin{tabular}[c]{@{}l@{}} \cite{9}\\
            \cite{10}   \\
            \cite{38}   \\
        \end{tabular}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Single-Source Shortest Path & \begin{tabular}[c]{@{}l@{}} \cite{11}\\
            \cite{41}   \\
            \cite{42}   \\
        \end{tabular}\\  
        \cline{2-3} 
        Path Analysis &  All-Pairs Shortest Path  & \begin{tabular}[c]{@{}l@{}} \cite{12}\\
            \cite{43,44}   \\
            \cite{45, 46}   \\
        \end{tabular}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Minimum Weight Spanning Tree & \begin{tabular}[c]{@{}l@{}} \cite{12, 13}\\
            \cite{47, 48}   \\
            \cite{49, 50}   \\
        \end{tabular}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Random Walk & \begin{tabular}[c]{@{}l@{}} \cite{14, 51}\\
            \cite{52, 53}   \\
            \cite{54}   \\
        \end{tabular}\\  
        \midrule

        Connectivity Analysis &  Kosaraju’s BFS and DFS  & \cite{15}\\
        \midrule

        \multirow{2}{*}{} & Label Propagation & \cite{16}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Louvain Modularity & \begin{tabular}[c]{@{}l@{}} \cite{20, 21}\\
            \cite{55, 56}   \\
            \cite{57} \\
        \end{tabular}\\ 
        \cline{2-3} 
        Community Analysis & Node Clustering Coefficient and Average Clustering Coefficient & \cite{22}   \\ 
        \cline{2-3}
        \multirow{2}{*}{} & Bron-Kerbosch & \begin{tabular}[c]{@{}l@{}} \cite{23}\\
            \cite{24}   \\
        \end{tabular}\\  
        \midrule

        \multirow{2}{*}{} & Degree Centrality & \cite{25}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Eigenvector Centrality & \begin{tabular}[c]{@{}l@{}} \cite{26}\\
            \cite{27}   \\
        \end{tabular}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Katz Centrality & \begin{tabular}[c]{@{}l@{}} \cite{28}\\
            \cite{58}   \\
        \end{tabular}\\  
        \cline{2-3} 
        Centrality Analysis &  PageRank Centrality  & \cite{29}\\
        \cline{2-3} 
        \multirow{2}{*}{} & Closeness Centrality & \begin{tabular}[c]{@{}l@{}} \cite{30}\\
            \cite{31}   \\
        \end{tabular}\\ 
        \cline{2-3} 
        \multirow{2}{*}{} & Betweenness Centrality & \cite{32}\\
        \midrule
...
\textbf{Graph Neural Network (GNN)} is deep learning applied on graph data for predictions of node labels and/or generating graphs and nodes.
...
The time complexity of SSSP is given by $O(v * e)$ while the space complexity is $O(v)$ where $v$ denotes vertices and $e$ is edges of the graph. It is used for detecting link failures in a network and come up with new links or routes in no time.
...
For a graph $G$ with $v$ vertices, the time complexity is $O(v^3)$. Its applications include computation of citations in citation networks hosted on the web, analyzing influential accounts on social networks and analyzing neurons in a neural network in the brain.
...
\begin{thebibliography}{9}
    ...
    \bibitem{52} T. T. Chitenderu, A. Maredza, and K. Sibanda, “The Random Walk Theory And Stock Prices: Evidence From Johannesburg Stock Exchange,” International Business \& Economics Research Journal (IBER), vol. 13, no. 6, pp. 1241–1250, Oct. 2014, doi: 10.19030/iber.v13i6.8918.
